...
impl<'a> ServerOutput<'a> {
    ...
    fn push_event_loop(&mut self) {
        ...
        if self.config.manual_event_loop {
            let send_events = self
                .config
                .casing
                .with("SendEvents", "sendEvents", "send_events");
    ...
    fn push_reliable_callback(&mut self, first: bool, ev: &EvDecl) {
        ...
        match ev.call {
            ...
            EvCall::SingleAsync => {
                self.push_line(&format!("task.spawn(events[{id}], player, value)"))
            }
    ...
    fn push_reliable(&mut self) {
        ...
        for ev in
            self.config.evdecls.iter().filter(|ev_decl| {
                ev_decl.from == EvSource::Client && ev_decl.evty == EvType::Reliable
            })
        {
    ...
    fn push_unreliable(&mut self) {
        ...
        for ev in self.config.evdecls.iter().filter(|ev_decl| {
            ev_decl.from == EvSource::Client && ev_decl.evty == EvType::Unreliable
        }) {
    ...
    fn push_callback_lists(&mut self) {
        ...
        for evdecl in self.config.evdecls.iter().filter(|ev_decl| {
            ev_decl.from == EvSource::Client
                && matches!(ev_decl.call, EvCall::ManyAsync | EvCall::ManySync)
        }) {
    ...
    fn push_return_fire_except(&mut self, ev: &EvDecl) {
        ...
        let fire_except = self
            .config
            .casing
            .with("FireExcept", "fireExcept", "fire_except");
